...
def _learn_rules(db, messages: list, session_id: str, now: str) -> int:
    ...
    for msg in user_messages:
        ...
        if any(word in content_lower for word in ['planet', 'sector', 'warp', 'credits', 'fleet', 'colony', 'population', 'allocated']):
            continue
        ...
        for rule_type, pattern, base_confidence in rule_patterns:
            ...
            for match in matches:
                ...
                if any(word in rule_lower for word in game_content_words):
                    continue

                # Skip if it's clearly not a rule
                ...
                if '`' in rule_text or '->' in rule_text or '==' in rule_text:
                    continue
                if rule_text.startswith('-') or (len(rule_text) > 1 and rule_text[0].isdigit() and rule_text[1] in '.):'):
                    continue
                ...
                learned += db.execute_write_func(CUSTODIAN_DB, upsert_rule)
