...
class DatabaseManager:
    ...
    def stop(self):
        ...
        self._shutdown.set()
        self._write_queue.put(None)  # Sentinel to wake up writer
    ...
    def _get_read_connection(self, db_name: str) -> sqlite3.Connection:
        ...
        Each thread gets its own connection for concurrent reads.
        """
        ...
        if db_name not in self._local.connections:
            ...
            conn = sqlite3.connect(str(db_path), check_same_thread=False, timeout=30.0)
            conn.row_factory = sqlite3.Row
            # Read connections also use WAL mode
            conn.execute("PRAGMA journal_mode=WAL")
            conn.execute("PRAGMA synchronous=NORMAL")
    ...
    def execute_write(self, db_name: str, sql: str, params: tuple = ()) -> int:
        ...
        Queues the operation and blocks until completion.
        Returns lastrowid for INSERT operations.
        """
    ...
    def execute_write_func(self, db_name: str, func: Callable[[sqlite3.Cursor], Any]) -> Any:
        ...
        The function receives a cursor and should perform all needed operations.
        Commit is automatic after the function returns.
        """
        if not self._initialized:
            self.start()
    ...
    def execute_read(self, db_name: str, sql: str, params: tuple = ()) -> List[sqlite3.Row]:
        ...
        Uses thread-local read connections for concurrent access.
